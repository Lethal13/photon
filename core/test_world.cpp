...
void init_world(world32_t *world)
{
    ...
    world->spheres = (sphere32_t *)malloc(sizeof(sphere32_t) * total_spheres);
    world->total_triangles = 0;
    world->triangles = 0;
    world->total_materials = total_materials;
    world->materials = (material32_t *)malloc(sizeof(material32_t) * total_materials);

    memset(world->spheres, 0, sizeof(sphere32_t) * total_spheres);
    memset(world->materials, 0, sizeof(material32_t) * total_materials);
...
void init_world(world64_t *world)
{
    ...
    world->total_spheres = total_spheres;
    world->spheres = (sphere64_t *)malloc(sizeof(sphere64_t) * total_spheres);
    world->total_triangles = 0;
    world->triangles = 0;
    world->total_materials = total_materials;
    world->materials = (material64_t *)malloc(sizeof(material64_t) * total_materials);

    memset(world->spheres, 0, sizeof(sphere64_t) * total_spheres);
    memset(world->materials, 0, sizeof(material64_t) * total_materials);
    ...
    // Ground sphere
    world->materials[current_material_index].material = MAT_DIFFUSE;
    world->materials[current_material_index].base_color = color3d_t(0.5, 0.5, 0.5);
    world->materials[current_material_index].roughness = 0.0;
    world->materials[current_material_index].refraction_index = 0.0f;

    world->spheres[current_sphere_index].center = point3d_t(0.0, -1000.0, 0.0);
    world->spheres[current_sphere_index].radius = 1000.0f;
    world->spheres[current_sphere_index].material_index = current_material_index;
    ...
    for (int a = -11; a < 11; ++a)
	{
        for (int b = -11; b < 11; ++b)
		{
			...
			vec3d_t tmp = center - point3d_t(4.0f, 0.2f, 0.0f);
            ...
            if (dot(tmp, tmp) > 0.9f)
			{
				...
				{
                    // Diffuse
					vec3d_t v1 = random_vec3d();
					vec3d_t v2 = random_vec3d();
                    world->materials[current_material_index].material = MAT_DIFFUSE;
                    world->materials[current_material_index].base_color = color3d_t(v1.x * v2.x, v1.y * v2.y, v1.z * v2.z);
                    world->materials[current_material_index].roughness = 0.0f;
                    world->materials[current_material_index].refraction_index = 0.0f;
				} else if (choose_mat < 0.95f) {
                    // Metal
					vec3f_t v1 = random(0.5f, 1.0f);
                    world->materials[current_material_index].material = MAT_METAL;
                    world->materials[current_material_index].base_color = color3d_t(v1.x, v1.y, v1.z);
                    world->materials[current_material_index].roughness = random_float(0.0f, 0.5f);
                    world->materials[current_material_index].refraction_index = 0.0f;
                } else {
                    // Glass
					world->materials[current_material_index].material = MAT_DIELECTRIC;
                    world->materials[current_material_index].base_color = color3d_t(1.0f, 1.0f, 1.0f);
                    world->materials[current_material_index].roughness = 0.0f;
                    world->materials[current_material_index].refraction_index = 1.5f;
                }
                world->spheres[current_sphere_index].center = center;
    ...
    // Three larger spheres
    world->materials[current_material_index].material = MAT_DIELECTRIC;
    world->materials[current_material_index].base_color = color3d_t(0.8f, 0.9f, 1.0f);
    world->materials[current_material_index].roughness = 0.0f;
    world->materials[current_material_index].refraction_index = 1.5f;

    world->spheres[current_sphere_index].center = point3d_t(0.0f, 1.0f, 0.0f);
    world->spheres[current_sphere_index].radius = 1.0f;
    world->spheres[current_sphere_index].material_index = current_material_index;
    ...
    world->materials[current_material_index].material = MAT_DIFFUSE;
    world->materials[current_material_index].base_color = color3d_t(0.4f, 0.2f, 0.1f);
    world->materials[current_material_index].roughness = 0.2f;
    world->materials[current_material_index].refraction_index = 0.0f;
    
	world->spheres[current_sphere_index].center = point3d_t(-4.0f, 1.0f, 0.0f);
    world->spheres[current_sphere_index].radius = 1.0f;
    world->spheres[current_sphere_index].material_index = current_material_index;
    ...
    world->materials[current_material_index].material = MAT_METAL;
    world->materials[current_material_index].base_color = color3d_t(0.7f, 0.6f, 0.5f);
    world->materials[current_material_index].roughness = 0.0f;
    world->materials[current_material_index].refraction_index = 0.0f;
        ...
        camera32_t *camera)
{
	...
	world->materials = (material32_t*)malloc(sizeof(material32_t) * materials_count);

	// PHOTON_ASSERT((obj_result.index_buffer_size % 3) == 0);

	world->total_triangles = (uint32_t)(obj_result.index_buffer_size / 3);
	world->triangles = (triangle32_t*)malloc(sizeof(triangle32_t) * world->total_triangles);

	material32_t material1 = {};
        ...
        camera64_t *camera)
{
    ...
    point3d_t look_from(0.0f, 0.0f, 8.0f);
    point3d_t look_at(0.0f, 0.0f, -1.0f);
    vec3d_t v_up(0.0f, 1.0f, 0.0f);
    ...
    initialize_camera64(camera_setup.aspect_ratio, 
                       camera_setup.width, 
                       camera_setup.height, 
                       look_from, 
                       look_at, 
	...
	world->total_materials = materials_count;
	world->materials = (material64_t*)malloc(sizeof(material64_t) * materials_count);

	// PHOTON_ASSERT((obj_result.index_buffer_size % 3) == 0);

	world->total_triangles = (uint32_t)(obj_result.index_buffer_size / 3);
	world->triangles = (triangle64_t*)malloc(sizeof(triangle64_t) * world->total_triangles);

	material64_t material1 = {};
	material1.material = MAT_DIFFUSE;
	material1.base_color = color3d_t(0.753, 0.647, 0.435);
